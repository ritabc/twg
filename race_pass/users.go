...
// Common errors that we will likely want to acccount for in the code.
// Any other errors are wrapped with context via the github.com/pkg/errors package and returned but are harder to use an if/switch to match
var (
	ErrNotFound = errors.New("race: resource counld not be located")
)

// User is an example user model. This typicall woudln't be defined in this package but is done here for simplicity
type User struct {
	ID      int
	Name    string
	Email   string
...
// Find will retrieve a user with the provided ID or return ErrNotFound if the user wasn't located. Other errors are wrapped with context but are otherwise wrapped as-is
...
// Delete will delete a user from the DB. If there is an error it will be wrapped and returned
...
func Spend(tx Transcation, userID int, amount int) error {
