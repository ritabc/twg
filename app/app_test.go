...
func TestApp_v2(t *testing.T) {
	...
	t.Run("custom built request", func(t *testing.T) {
		r := signedInRequest(t, http.MethodGet, server.URL+"/admin", nil)
...
func signedInClient(t *testing.T, baseURL string) *http.Client {
	...
	// Our client has a cookie jar, but it has no session cookie. By logging in we can ensure that it gets set
	loginURL := baseURL + "/login"
	r, err := http.NewRequest(http.MethodPost, loginURL, nil)
	if err != nil {
	...
	t.Logf("Cookies: %v", client.Jar.Cookies(r.URL))
	return client
}
